...
SESSION_SECRET = os.getenv("SESSION_SECRET", "").strip() or os.getenv("SECRET_KEY", "").strip() or "change-me-session-key"
...
def build_activity_heatmap(matches, days=ACTIVITY_DAYS):
    ...
    for match in matches:
        ...
        day = datetime.datetime.utcfromtimestamp(timestamp).date()
...
def normalize_stratz_match_row(match_node, player_node):
    ...
    return {
        ...
        "duration": duration,
        "game_mode": to_int(match_node.get("gameMode"), 0),
...
async def fetch_stratz_player_payload(client, player_id):
    ...
    query StratzPlayerCore($playerId: Long!, $take: Int!, $heroTake: Int!) {
      player(steamAccountId: $playerId) {
        ...
        allModes: matchesGroupBy(request: { take: 1, playerList: SINGLE, groupBy: STEAM_ACCOUNT_ID }) {
          ... on MatchGroupBySteamAccountIdType {
            matchCount
            winCount
          }
        }
        turboModes: matchesGroupBy(
          request: { take: 1, playerList: SINGLE, groupBy: STEAM_ACCOUNT_ID, gameModeIds: [23] }
        ) {
          ... on MatchGroupBySteamAccountIdType {
            matchCount
            winCount
...
async def get_player_data_from_stratz(client, player_id):
    ...
    all_modes_row = first_list_object(player_payload.get("allModes"))
    total_matches = to_int(all_modes_row.get("matchCount"), 0)
    wins_all = to_int(all_modes_row.get("winCount"), 0)
    ...
    turbo_row = first_list_object(player_payload.get("turboModes"))
    turbo_total = to_int(turbo_row.get("matchCount"), 0)
    turbo_wins = to_int(turbo_row.get("winCount"), 0)
    if turbo_total <= 0:
    ...
    for match in recent_matches_source:
        ...
        mode_id = to_int(detail.get("gameMode"), to_int(match.get("game_mode"), 0)) if isinstance(detail, dict) else to_int(match.get("game_mode"), 0)
...
async def resolve_first_match_date(client, player_id, total_matches, recent_matches):
    # Try ascending sort approach first (fastest)
    try:
        ascending = await fetch_json(
            client,
            f"{OPEN_DOTA_API}/players/{player_id}/matches?limit=1&sort=start_time&ascending=1&significant=0",
            [],
        )
        if isinstance(ascending, list) and ascending:
            return format_match_date(ascending[0].get("start_time"))
    except Exception:
        pass

    # Fallback: use oldest match from recent_matches
    ...
    # Last resort: try one fetch at total_matches - 1 offset
    if total_matches > 0:
        try:
            oldest_by_hint = await fetch_match_at_offset(client, player_id, min(total_matches - 1, 100))
            if oldest_by_hint:
                return format_match_date(oldest_by_hint.get("start_time"))
        except Exception:
            pass
...
async def home(request: Request):
    ...
    return templates.TemplateResponse("home.html", {"request": request, "steam_user": steam_user})
